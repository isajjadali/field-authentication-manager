...
function extractKeys(url = [], requiredFields = {}) {
    ...
    if (!extractedJson) {
        ...
        const finestExtractedJson = Object.keys(requiredFields)
            .map((route) => {
                const splittedRoute = route
                    .split(pathSeperator)
                    .filter(item => item != '');
                return splittedRoute.filter(item => item != '').length === url.length ? splittedRoute : [];
            })
            .filter(route => route.length)
            .filter(route => route.every((item, index) => (item.indexOf(':') === -1 ? item === url[index] : true)))[0];

        return finestExtractedJson && (requiredFields[finestExtractedJson.join(pathSeperator)] || requiredFields[`/${finestExtractedJson.join(pathSeperator)}`]);
    }
