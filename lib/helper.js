...
function removeOptionalKeys(required = {}, request = {}) {
    return keys(required).forEach(key => required[key].isOptional && (typeof request[key] === 'undefined' || request[key] === null) && (delete required[key], delete request[key]));
}
...
function removeExtraKeys(required = {}, request = {}) {
    ...
    if (required.removeableKeys) {
        request = omit(request, required.removeableKeys);
        delete required.removeableKeys;
    }
...
function errorMaker(fields = []) {
    return (fields.length && `REQUIRED:: ${fields.join(', ')} ${(fields.length > 1 && 'are required') || 'is required'}!`) || '';
}
 ...
 * @return {boolean} True if its windows
 * */
function isWindows() {
