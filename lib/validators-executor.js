...
module.exports.validatorsExecutor = (required = {}, request = {}) => {
    const fuctionShouldPerform = filterAlreadyRegisteredValidators(required);
    ...
    const errors = chain(RequestBox)
        .values()
        .map(boxType => compact(map(fuctionShouldPerform, (func) => {
            if (!isErrorFound) {
                if (boxType === RequestBox.body) {
                    const error = func(cloneDeep(requiredBody), requestBody, RequestBox.body);
                    if (error) {
                        isErrorFound = true;
                        return {
                            error,
                    ...
                    const error = func(cloneDeep(requiredQuery), requestQuery, RequestBox.query);
                    if (error) {
                        isErrorFound = true;
                        return {
                            error,
